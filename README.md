...
- CLI：`jj-bookmark-cli/` 构建出二进制 `jj-bookmark`；`apply <URL|ID>` 统一保存 / 编辑，`apply <ID> --delete` 删除，`ls [KEYWORD]` 统一列表 / 搜索；全局选项只有 `--source <NAME>`（子命令前后均可写），每条命令恒作用于单一 source，省略即 `default`（= `apply <URL>` 的落点）；其他命令见 `jj-bookmark --help`（仅英文）。
